  ...
  return (
    <header
      className={`w-full sticky top-0 z-40 transition-all duration-1000 ${
        scrolled
          ? "bg-white/95 backdrop-blur supports-[backdrop-filter]:bg-white/70 shadow-md"
          : "bg-white shadow-sm"
      } `}
    >
      <div
        className={`w-[80%] mx-auto flex items-center justify-between transition-all duration-300 ${
          scrolled ? "h-16 py-3" : "h-20 py-5"
        }`}
      >
        <div className="flex items-center gap-6">
          <Link href={"/"} className="select-none">
            <h1 className="text-3xl text-red-600 font-bold tracking-tight">
              E-Shop
            </h1>
          </Link>
        </div>
        ...
        <div className="flex items-center gap-5">
          ...
          <Link
            href={"/profile"}
            className={`border-2 rounded-full border-gray-300 w-10 h-10 p-1 flex items-center justify-center hover:border-blue-500 transition-colors ${
              pathname === "/profile" ? "ring-2 ring-blue-300" : ""
            }`}
            aria-label="Profile"
          >
            <svg
          ...
          <Link
            href={"/cart"}
            className={`relative hover:text-blue-600 transition-colors ${
              pathname.startsWith("/cart") ? "text-blue-600" : "text-gray-600"
            }`}
            aria-label="Cart"
          >
